...
def film_detail(request, id):
    ...
    subtitles = Subtitle.objects.filter(film=id).order_by('start_time').values('text', 'start_time', 'end_time', 'character')
    subtitles_json = json.dumps(list(subtitles))
    if film.trailer_url:
        video_url = Film.objects.filter(id=id)[0].trailer_url
        url_id = _url_id(video_url)
    else:
        url_id = None

    return render(request, 'films/film/detail.html',
                  {'film': film, 'subtitles': subtitles_json, 'url_id': url_id})


def _url_id(url):
    return url[url.find('embed/')+6:]
